...
const URL =
  "https://api.opencagedata.com/geocode/v1/json?key=477650a203ff446ba31a00fa8d53afdd&q=";
...
app.put("/entries", (req, res) => {
  if (req.body) {
    ...
    const index = rescueModel.entries.findIndex(
      (entry) => entry.ID === entryInput.ID
    );
    if (index !== -1) {
      // we found it
      const existing = rescueModel.entries[index];

      const updatedEntry = {
        ID: existing.ID, // Keep the same ID
        name: entryInput.name || existing.name,
        address: entryInput.address || existing.address,
        number: entryInput.number || existing.number,
        category: entryInput.category || existing.category,
